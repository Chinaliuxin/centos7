...
public:
    static int Split(std::string& src,const std::string seg,std::vector<std::string>& list)
  {
    ...
    static std::string GetErrDesc(std::string & code)
    {
      ...
      return "Unknow Error";//代码中没有的错误码，就返回不知道
...
class HttpTask
{
  ...
  public:
	...
	//HttpTask tt;tt.sethttptask(cli_sock,func)====>线程池
};
...
class ThreadPool
{
  ...
  private:
    ...
    static void* start(void* arg)//完成获取线程的任务，执行任务，避免this指针
    {
  ...
  public:
    ...
    bool ThreadPoolInit()//线程创建，互斥锁，条件变量初始
    {
      while(_num<_max)
      {
          pthread_t tid;
          ...
          pthread_create(&tid,NULL,ThreadPool::start,(void*)this);//thread ti(func,参数)  ti.join
          _num++;
      }
      return 1;
    }

    bool PushTask(HttpTask& ht)//线程安全的任务入队
    {
      Lock();
      if(stop)
      {
...
class HttpResponse
{
  ...
  public:
    ...
    bool ProcessFile(RequestInfo &info)//文件下载功能处理
    {
      std::cout<<std::endl;
      ...
      int fd=open(info._path_phys.c_str(),O_RDONLY);//O_RDONLY只读
      if(fd<0)
      {
        info.SetError(400);
        ErrHandle(info);
    ...
    bool ProcessList(RequestInfo& info)//文件列表功能处理
    {
      //发送头部
      std::string rqs_head;
      rqs_head=info._version+" 200 OK\r\n";
      ...
      //发送正文
      std::string rqs_body;
      rqs_body="<html><head><title>liuxin' computer Index: /</title>";
    ...
    {
		int out[2];//in管道，用户（父进程）向子进程传递正文
		int in[2];//out管道，从子进程中读数据
      if(pipe(in) < 0)
      {
        info.SetError(500);
        perror("pipe error");
      ...
      else if(pid==0)
      {
        ...
        dup2(in[0],0);//参考开源，把标准输入，标准输出重定到管道端，子进程从标准输入读
        dup2(out[1],1);//子进程打印传递给父进程
        std::cerr<<info._path_phys<<std::endl;
        if(execl(info._path_phys.c_str(),info._path_phys.c_str(),NULL)<0)//execl程序替换，替换失败，直接退出
	 ...
	 //将处理结果组织http数据，相应给客户端
      auto it=info._hdr_list.find("Content-Length");//看有没有正文
